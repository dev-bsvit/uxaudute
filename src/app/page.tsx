...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const checkUserBalance = async () => {
      try {
        ...
        if (session) {
          ...
          // Нет сессии - пользователь не залогинен, показываем модалку
          console.log('👉 Пользователь НЕ залогинен (нет сессии), показываем модалку...')
    ...
    // Запускаем проверку через 1.5 секунды после загрузки страницы
    // (даем больше времени для загрузки сессии)
    setTimeout(checkUserBalance, 1500)
