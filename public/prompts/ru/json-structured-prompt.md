  ...
  "problemsAndSolutions": [
    {
      ...
      "impact": "high",
      "why": "[3-5 предложений: ПОЧЕМУ это проблема для ЭТОГО конкретного экрана. Включи: (1) Описание текущего состояния с конкретикой из скрина (цвета, размеры, расположение), (2) Ссылку на UX-исследование/принцип (Nielsen, WCAG, eye-tracking, A/B тесты), (3) Процент влияния на поведение пользователей (например '60% пользователей пропускают'), (4) Бизнес-последствия (влияние на конверсию, отказы, поддержку). НЕ копируй этот пример - анализируй РЕАЛЬНЫЙ скриншот!]",
      "fix": "[3-5 предложений: КАК исправить с техническими деталями. Включи: (1) Конкретные значения (#HEX цвета, px размеры, WCAG коэффициенты), (2) CSS-свойства и анимации (transform, transition, shadow), (3) Технические решения (regex, aria-live, event handlers), (4) Адаптивность (desktop/mobile различия), (5) Edge cases (loading states, errors). Будь максимально конкретен с кодом!]",
      "result": "[1-2 предложения: Ожидаемый результат с числовыми диапазонами (25-30%) И временными рамками (первые 2 недели, через месяц). Основывайся на похожих кейсах из индустрии.]"
    },
    {
...
3. **⚠️ КРИТИЧЕСКИ ВАЖНО - АНАЛИЗИРУЙ ТОЛЬКО РЕАЛЬНЫЙ СКРИНШОТ:**
   ...
   - ❌ ЗАПРЕЩЕНЫ общие фразы типа "Низкая видимость CTA кнопки", "Увеличить контраст кнопки"
   - ❌ ЗАПРЕЩЕНО писать про "контраст", "размер кнопки", если это НЕ реальная проблема на скриншоте
   - ❌ НЕ используй шаблонные решения - каждое исправление должно быть специфичным для ЭТОГО экрана
   - ✅ ОБЯЗАТЕЛЬНО анализируй РЕАЛЬНЫЕ элементы, которые ВИДНЫ на скриншоте
   - ✅ Называй проблемы по КОНКРЕТНЫМ элементам: "Кнопка 'Оплатити' [конкретная проблема]"
   - ✅ Если кнопка хорошо выделена и контрастна - НЕ пиши про это как про проблему!
...
5. **Пиши ДЕТАЛЬНЫЕ, ЦЕННЫЕ ответы**:
   ...
   - `fix`: 3-5 предложений с конкретными метриками (ПРИМЕРЫ метрик: #10B981 цвета, 48px размеры, WCAG 4.5:1 коэффициенты - но используй РЕАЛЬНЫЕ значения с экрана!)
   - `result`: Включай диапазоны чисел (25-30%) И временные рамки (первые 2 недели)
6. **Включай ссылки на реальные UX-исследования**: исследования Нильсена, рекомендации WCAG, данные eye-tracking, результаты A/B тестов
