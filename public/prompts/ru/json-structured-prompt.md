...
3. **ВАЖНО - АНАЛИЗИРУЙ РЕАЛЬНЫЙ СКРИНШОТ**:
   - ❌ НЕ копируй примеры из промпта (про контраст кнопки, email валидацию, заголовки)
   - ✅ Находи РЕАЛЬНЫЕ проблемы на ЭТОМ конкретном скриншоте
   - ✅ Каждая проблема должна быть УНИКАЛЬНОЙ и основанной на видимых элементах
