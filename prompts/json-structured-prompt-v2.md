...
- **ОБЯЗАТЕЛЬНО: uxSurvey.questions должен содержать РОВНО 5 вопросов - НЕ МЕНЬШЕ!**
- **ОБЯЗАТЕЛЬНО: audience.fears должен содержать РОВНО 5 страхов - НЕ МЕНЬШЕ!**
- **ОБЯЗАТЕЛЬНО: problemsAndSolutions должен содержать РОВНО 3 проблемы - НЕ МЕНЬШЕ!**
- **ОБЯЗАТЕЛЬНО:** Каждая проблема должна содержать ВСЕ поля: element, problem, principle, consequence, recommendation, expectedEffect, priority
- **ОБЯЗАТЕЛЬНО:** Используйте разные категории вопросов: clarity, usability, accessibility, conversion, trust
- **ОБЯЗАТЕЛЬНО:** Каждая проблема должна иметь разный приоритет: high, medium, low
- **ОБЯЗАТЕЛЬНО:** Если не генерируете минимальное количество - это ОШИБКА!
