...
## Вихід
**КРИТИЧНО ВАЖЛИВО: 
1. Відповідай ТІЛЬКИ у форматі JSON
2. НЕ додавай жодного тексту до або після JSON
3. НЕ обгортай JSON у markdown блоки (```json)
4. НЕ додавай пояснення або коментарі
5. Починай відповідь одразу з символу { та закінчуй символом }
6. Переконайся, що JSON валідний та повний
7. Всі рядки мають бути в лапках, числа без лапок
8. Масиви в квадратних дужках [], об'єкти в фігурних дужках {}**
...
- **КРИТИЧНО:** НІКОЛИ не пропускайте поля consequence або expectedEffect - вони ОБОВ'ЯЗКОВІ
...
```json
{
  ...
  "audience": {
    "fears": [
      "Страх 1 - коротке опис",
      "Страх 2 - коротке опис", 
      "Страх 3 - коротке опис",
      "Страх 4 - коротке опис",
      "Страх 5 - коротке опис"
    ]
  },
  "problemsAndSolutions": [
    {
...
**Відповідай ТІЛЬКИ у форматі JSON українською мовою.**

**ФІНАЛЬНЕ НАГАДУВАННЯ: Кожна проблема в problemsAndSolutions ОБОВ'ЯЗКОВО має включати поля consequence та expectedEffect. Не пропускайте ці поля за жодних обставин.**
