  ...
  "uxSurvey": {
    ...
    "questions": [
      ...
      },
      {
        ...
        "confidence": 80,
        "category": "usability",
        "principle": "Findability (Находимость)",
        "explanation": "принцип доступности информации"
      },
      {
        "id": 3,
        "question": "Насколько доверяете интерфейсу?",
        "options": [
          "A) Полностью доверяю",
          "B) Частично доверяю",
          "C) Не доверяю"
        ...
        "confidence": 75,
        "category": "trust",
        "principle": "Trust & Credibility (Доверие)",
        "explanation": "важность доверия пользователей"
      },
      {
        "id": 4,
        "question": "Насколько понятны действия с элементами?",
        "options": [
          "A) Очень понятны",
          "B) Частично понятны",
          "C) Непонятны"
        ...
        "confidence": 82,
        "category": "usability",
        "principle": "Affordance (Предсказуемость)",
        "explanation": "ясность возможных действий"
      },
      {
        "id": 5,
        "question": "Насколько быстро можно выполнить задачу?",
        "options": [
          "A) Очень быстро",
          "B) Умеренно быстро",
          "C) Медленно"
        ...
        "confidence": 78,
        "category": "efficiency",
        "principle": "Efficiency (Эффективность)",
        "explanation": "скорость выполнения задач"
      },
      {
        "id": 6,
        "question": "Насколько удобно на мобильном устройстве?",
        "options": [
          "A) Очень удобно",
          "B) Умеренно удобно",
          "C) Неудобно"
        ...
        "principle": "Responsive Design (Адаптивность)",
        "explanation": "адаптация под разные устройства"
      },
      {
        "id": 7,
        "question": "Насколько привлекателен дизайн?",
        "options": [
          "A) Очень привлекателен",
          "B) Умеренно привлекателен",
          "C) Непривлекателен"
        ],
        "scores": [45, 40, 15],
        "confidence": 68,
        "category": "aesthetics",
        "principle": "Visual Design (Визуальный дизайн)",
        "explanation": "эстетическое восприятие интерфейса"
      }
    ],
...
- **questions**: Создайте МИНИМУМ 5-7 релевантных вопросов для конкретного экрана. Если экран сложный, можно больше (до 10-12 вопросов)
- **scores**: Экспертные оценки в процентах (сумма = 100)
- **confidence**: Уверенность в каждом вопросе (60-95%)
- **category**: Тип вопроса (clarity, trust, usability, conversion, accessibility, content, aesthetics, efficiency, findability)
- **principle**: Конкретный UX-принцип с названием на английском и русском
- **explanation**: Объяснение релевантности вопроса для данного интерфейса

**ВАЖНО**: Адаптируйте вопросы под конкретный тип экрана:
- Для форм: вопросы о ясности полей, валидации, ошибках
- Для дашбордов: вопросы о навигации, информационной архитектуре
- Для лендингов: вопросы о конверсии, доверии, призыве к действию
- Для мобильных: вопросы об удобстве использования пальцами, читаемости
...
- **Генерируйте МИНИМУМ 5-7 вопросов** в uxSurvey.questions
- **Адаптируйте вопросы** под конкретный интерфейс на скриншоте
- **Используйте разнообразные категории**: clarity, usability, trust, accessibility, efficiency, aesthetics
- **Каждый вопрос должен быть релевантным** для данного типа экрана
- **Не ограничивайтесь базовыми вопросами** - создавайте специфичные для контекста
